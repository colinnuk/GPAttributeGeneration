...
public class Population
{
    ...
    public void calculateFitness(EnumFitnessEvaluationMethod evalMethod, Instances data, Classifier classifier)
    {
        ...
        else if (evalMethod == EnumFitnessEvaluationMethod.FSAR)
        {
            calculateQRFitness(gpDataset, data.numAttributes());
        }
        else if (evalMethod == EnumFitnessEvaluationMethod.HYBRID)
        {
            //Calculates fitness using a hybrid of QR and J48
            calculateHybridFitness(gpDataset, classifier);
        }
    ...
    /**
     * Calculates fitness using Fuzzy Rough Set Attribute Reduction (QuickReduct).
     * @param gpDataset The dataset with the additional GP attributes
     * @param originalAttributes The original number of attributes from the dataset
     */
    private void calculateQRFitness(Instances gpDataset, int originalAttributes)
    {
        //Set up the attribute selector
        AttributeSelection attSel = new AttributeSelection();
        FuzzyRoughSubsetEval frse = new FuzzyRoughSubsetEval();
        HillClimber hillClimb = new HillClimber();
        attSel.setEvaluator(frse);
        attSel.setSearch(hillClimb);

        try
        {
            //Perform attribute selection on the dataset
            attSel.SelectAttributes(gpDataset);
            //Get the results
            int[] selectedAttrs = attSel.selectedAttributes();
            Arrays.sort(selectedAttrs);

            //Set the fitness for each member of the population
            for(int i = 0; i < population.size(); i++)
            {
                //If number of selected attrs is same as it was, then no 'reduct' has been found with those attributes
                //If i is one of the selected attributes (taking into account the original number of attributes), give a good fitness
                if(selectedAttrs.length <= originalAttributes && Arrays.binarySearch(selectedAttrs, i + originalAttributes) >= 0)
                {
                    population.get(i).setTreeAccuracy(1);
                }
                else
                {
                    population.get(i).setTreeAccuracy(0);
                }
            }
    ...
    /**
     * Calculates fitness using a hybrid of J48 and QR
     * @param gpDataset dataset with all individuals from the population added
     * @param classifier Classifier to use (J48)
     */
    private void calculateHybridFitness(Instances gpDataset, Classifier classifier)
    {
        //Do a reduct first
        //Set up the attribute selector
        AttributeSelection attSel = new AttributeSelection();
        ...
        try
        {
            ...
            //Remove all unselected attributes from the dataset
            //if no attributes were selected (ignoring the class attribute), the whole dataset is passed to the tree builder
            if(selectedAttrs.length > 1)
            {
                int classIndex = gpDataset.classIndex();
                //Do the loop 'backwards' so as not to affect attribute indices until after they have been dealt with
                for(int i = gpDataset.numAttributes() - 1; i >= 0; i-- )
                {
                    //If the attribute is not on the list of selected attributes, remove it
                    if(Arrays.binarySearch(selectedAttrs, i) < 0 && classIndex != i)
                    {
                        gpDataset.deleteAttributeAt(i);
                    }
                }
            }
        }
        ...
        {
            //Even if an exception occured while applying the reduct; run the reduced dataset thru the J48 fitness calculator
            calculateJ48Fitness(gpDataset, classifier);
        }
    }
